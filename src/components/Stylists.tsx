...
function StylistCard({ stylist, index }: { stylist: typeof stylists[0]; index: number }) {
  ...
  return (
    ...
    >
      {/* Portrait - properly constrained and centered */}
      <div className="relative mb-8 overflow-hidden rounded-2xl mx-auto max-w-[420px]">
      ...
      <h3 className="font-serif text-2xl text-charcoal mb-1 tracking-tight">{stylist.name}</h3>
      <p className="text-gold text-[10px] tracking-[0.15em] uppercase mb-5 font-light">{stylist.title}</p>

      <div className="max-w-[320px] mx-auto">
        <p className="text-stone font-light leading-relaxed text-[14px]">
          {stylist.bio}
          {!expanded && (
            <button
              onClick={() => setExpanded(true)}
              className="ml-1.5 text-blush hover:text-blush-dark text-[14px] font-normal transition-colors duration-200 cursor-pointer underline underline-offset-2 decoration-blush/30"
            >
              Read more
            </button>
          )}
        ...
        {expanded && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
            <p className="text-stone font-light leading-relaxed text-[14px] mt-3">{stylist.extended}</p>
            <button onClick={() => setExpanded(false)} className="mt-2 text-stone-light hover:text-blush text-xs font-light transition-colors duration-200 cursor-pointer">
              Show less
            </button>
          </motion.div>
        )}
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-3 mt-8">
        <a href={stylist.bookingUrl} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center justify-center px-7 py-3 bg-blush text-white text-[11px] tracking-[0.12em] uppercase rounded-full hover:bg-blush-dark hover:scale-[1.03] transition-all duration-300 active:scale-[0.97]">
          {stylist.bookingLabel}
        </a>
        <a href={stylist.instagramUrl} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center justify-center px-7 py-3 border border-beige text-stone text-[11px] tracking-[0.12em] rounded-full hover:border-blush-light hover:text-blush-dark hover:scale-[1.03] transition-all duration-300">
          {stylist.instagram}
        </a>
      </div>
    </motion.div>
...
export default function Stylists() {
  return (
    <section id="stylists" className="py-16 md:py-20 lg:py-28 bg-cream">
      <div className="w-full max-w-[1280px] mx-auto px-6">
        ...
        >
          ...
          <h2 className="font-serif text-3xl md:text-5xl font-light text-charcoal tracking-tight max-w-[800px] mx-auto">Two stylists. Your full attention.</h2>
        </motion.div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-20 max-w-[960px] mx-auto">
