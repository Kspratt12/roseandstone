    ...
    <section id="booking" className="py-20 md:py-28 bg-white">
      <div className="section-container-narrow">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {bookingOptions.map((option, i) => (
            <motion.div
              ...
              className="bg-cream rounded-2xl p-6 border border-beige/60 hover:border-blush-light/40 transition-all duration-500"
            >
              {/* Centered content */}
              <div className="flex flex-col items-center text-center">
                <div className="w-[68px] h-[68px] rounded-full overflow-hidden mb-4 ring-2 ring-beige/50">
                  <Image src={option.image} alt={option.name} width={68} height={68} quality={85} className="w-full h-full object-cover" />
                </div>

                <h3 className="font-serif text-xl text-charcoal">{option.name}</h3>
                <p className="text-gold text-[10px] tracking-[0.12em] uppercase mt-0.5 mb-3 font-light">{option.title}</p>
                <p className="text-stone/60 text-[12px] mb-5 font-light">{option.focus}</p>

                <a
                  href={option.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block w-full py-3 bg-blush text-white text-center text-[12px] tracking-wider uppercase rounded-full hover:bg-blush-dark transition-all duration-300 active:scale-[0.97]"
                >
                  {option.buttonText}
                </a>
                <p className="text-[10px] text-stone-light mt-2 font-light">{option.note}</p>

                <div className="mt-3 pt-3 border-t border-beige/30 w-full flex flex-col items-center gap-0.5 text-[12px] text-stone/60 font-light">
        ...
        >
          <button
            onClick={onBookClick}
            className="inline-flex items-center gap-2 px-6 py-2.5 bg-charcoal text-white text-[11px] tracking-wider uppercase rounded-full hover:bg-soft-black transition-colors active:scale-[0.97] cursor-pointer"
          >
          ...
          <p className="text-stone/50 text-[12px] font-light">
            ...
            <a href="#contact" className="text-blush hover:text-blush-dark underline underline-offset-4 decoration-blush/20 transition-colors">Tell us what you&apos;re looking for</a>
          </p>
        </motion.div>
      </div>
    </section>
